...
namespace Ecommerce_Apis.CartModule.Repositories
{
    public class WishListRepositories : IWishListRepositories
    {
        ...
        public async Task<bool> CreateWishList(AddToCartRequestDTO request, string userId)
        {
            ...
            bool productExists = await _context.WishLists
                .AsNoTracking()
                .AnyAsync(list => list.ProductId == request.ProductId && list.UserId == userId);

            if (productExists)
                return false;

            // Create and add a new wishlist entry
            var wishList = new WishList
            {
                UserId = userId,
                ProductId = request.ProductId,
                CouponId = request.CouponId, // Ensure CouponId is optional if nullable
                Quantity = request.Quantity,
                CreatedAt = DateTime.UtcNow // Add a timestamp if the model supports it
            };
        ...
        public async Task<bool> DeleteAllWishLists(string userId)
        {

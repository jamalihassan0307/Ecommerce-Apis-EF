...
namespace Ecommerce_Apis.OrderModule.Repositories
{
    public class OrderRepositories : IOrderRepositories
    {
        ...
        public async Task<dynamic> CreateOrder(CreateOrderRequest request, string userId, string couponcode)
        {
            // Use EF Core's execution strategy for retries
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    // Step 1: Check if products exist and have sufficient stock
                    ...
                    foreach (var item in request.Items)
                    {
                        ...
                        if (couponcode != null)
                        {
                            var coupon = await _context.Coupons
                                .FirstOrDefaultAsync(c => c.Code == couponcode && c.ExpirationDate > DateTime.UtcNow);
                        ...
                        var orderItem = new OrderItem
                        {
                            ...
                            Price = itemPrice,
                            CouponId = item.CouponId == 0 ? null : item.CouponId,
                        };
